...
async def hunt(as_client: httpx.AsyncClient, email_address: str, json_file: Path = None):
    ...
    # gb.rc.print(f"Customer ID : {x if (
        # x := target.extendedData.dynamiteData.customerId) else '[italic]Not found.[/italic]'}")

    # gb.rc.print(f"\n🌐 Google Plus Extended Data\n", style="cyan")

    # print(f"Entreprise User : {
        #   target.extendedData.gplusData.isEntrepriseUser}")
    ...
    if cal_found:
        ...
        if calendar_events.items:
            if "PROFILE" in target.names:
                gcalendar.out(calendar, calendar_events,
                              email_address, target.names[container].fullname)
            else:
